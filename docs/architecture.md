...
| Crate           | Description                                                      |
| -----------     | -----------                                                      |
| fugue-stdx      | Extensions to the standard library (similar to [`rust-analyzer`'s](https://github.com/rust-lang/rust-analyzer/blob/ea413f67a8f730b4211c09e103f8207c62e7dbc3/crates/stdx/Cargo.toml#L5)) |
| fugue-core      | Core editing primitives, functional.                             |
| fugue-lsp       | Language server client                                           |
| fugue-lsp-types | Language Server Protocol type definitions                        |
| fugue-dap       | Debug Adapter Protocol (DAP) client                              |
| fugue-event     | Primitives for defining and handling events within the editor    |
| fugue-loader    | Functions for building, fetching, and loading external resources |
| fugue-view      | UI abstractions for use in backends, imperative shell.           |
| fugue-term      | Terminal UI                                                      |
| fugue-tui       | TUI primitives, forked from tui-rs, inspired by Cursive          |


This document contains a high-level overview of Fugue internals.
...
in `fugue-term/src/ui`. Some components like `Popup` and `Overlay` can take
...
The `fugue-event` crate defines primitives for defining and acting on events
...
`fugue-event` has tools for defining events and registering _hooks_ which run
any time an event is emitted. `fugue-event` also provides `AsyncHook` - a tool
...
editor-wide events can be sent to hooks with `fugue_event::dispatch`.
